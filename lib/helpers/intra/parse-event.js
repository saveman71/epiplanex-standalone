...
var getOfficialId = function(intraEvent) {
  var matches = intraEvent.codeevent && intraEvent.codeevent.match(/event-([0-9]+)/);
...
var parseOfficalEvent = function(intraEvent) {
  ...
  if(intraEvent.rdv_group_registered || intraEvent.rdv_indiv_registered) {
    ...
    dates = dates.split('|');
    if(dates.length === 2) {
      newEvent.start = new Date(dates[0]);
      newEvent.end = new Date(dates[1]);
    }
  }

  // Did we succeed in finding a better date ? If not, get the default ones (start and end)
  if(!newEvent.start) {
    newEvent.start = new Date(intraEvent.start);
    newEvent.end = new Date(intraEvent.end);
  }
...
var parsePersoEvent = function(intraEvent) {
  ...
  newEvent.start = intraEvent.start;
  newEvent.end = intraEvent.end;
};
